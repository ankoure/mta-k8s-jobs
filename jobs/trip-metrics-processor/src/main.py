    ...
    for stop_pair in stops:
        params = {
            ...
            "start_date": datetime.strftime(
                current_date, constants.DATE_FORMAT_BACKEND
            ),
            "end_date": datetime.strftime(
                current_date + delta - timedelta(days=1), constants.DATE_FORMAT_BACKEND
            ),
        }
...
def format_tt_objects(
    ...
    for current_date in date_range:
        ...
        if metrics and is_valid_entry(metrics, expected_num_entries, current_date):
            ...
            new_speed_object["miles_covered"] = metrics["count"] * Decimal(
                route_metadata["length"]
            )
            new_speed_object["track_mileage"] = Decimal(route_metadata["length"])
            new_speed_object["total_time"] = (
                round(metrics["mean"], 1) * metrics["count"]
            )
...
def populate_daily_table(
    start_date: datetime, end_date: datetime, line: str, route: str | None
):
    """Populate DeliveredTripMetrics table. Calculates median TTs and trip counts for all days between start and end dates."""
    print(
        f"populating DeliveredTripMetrics for Line/Route: {line}/{route if route else '(no-route)'}"
    )
    ...
    while current_date < end_date.date():
        ...
        date_range = get_date_range_strings(
            current_date, current_date + delta - timedelta(days=1)
        )
